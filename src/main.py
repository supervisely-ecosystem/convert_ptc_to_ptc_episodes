...
progress = sly.tqdm_sly(
    desc="Processing annotations", total=sum(ds.items_count for ds in g.project_datasets)
)
ds_names_to_anns = {}
ds_names_to_frames = {}
...
progress = sly.tqdm_sly(
    desc=f"Converting point clouds dataset [{dataset.name}] into episode",
    total=len(g.project_datasets),
)
